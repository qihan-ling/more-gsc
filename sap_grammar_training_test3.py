...
net = gsc.load_model('sap_g1_model_orig.pkl')
...
gsc.plot_train_result(
    net, savefilename_prefix='sap_g1_model_orig', legend=True, linewidth=1.5)
...
plt.ylim([0, 1.05])
plt.tight_layout()
plt.savefig('sap_g1_model_orig_parsing.png',
...
def plot_sentence_treelets(net, sent, sent_idx, target):
    ...
    plt.tight_layout()
    filename = f'sap_g1_model_orig_S{sent_idx}_{word_seq.replace(" ", "_")}.png'
