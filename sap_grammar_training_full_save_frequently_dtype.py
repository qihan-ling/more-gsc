...
import only_gscnet_speedup_sap_dtype as gsc  # Use memory-optimized version with float32
import numpy as np
import time
...
# Print memory statistics to verify float32 savings
gsc.print_memory_stats(net, label="AFTER INITIALIZATION")
...
for epoch_block in range(n_epochs // 1):
    net.train2(
...
gsc.plot_train_result(
    net, savefilename_prefix='sap_1k_model_sparse_nocompres', legend=True, linewidth=1.5)
