...
# ============================================================================
# Use float32 for 50% memory reduction with minimal precision loss
# Critical operations (gradients) can optionally use float64 for stability
DTYPE_STORAGE = np.float32      # For weights, activations (50% memory savings)
DTYPE_COMPUTE = np.float64      # For gradient accumulation (numerical stability)
DTYPE_SPARSE = np.float32       # For sparse matrices (main memory consumer)
print(f"Memory optimization: storage={DTYPE_STORAGE.__name__}, compute={DTYPE_COMPUTE.__name__}, sparse={DTYPE_SPARSE.__name__}")
...
def print_memory_stats(net, label=""):
    """Print memory usage statistics for the GscNet model.
    
    Useful for comparing float64 vs float32 memory footprints.
    """
    import sys
    ...
    # Weight matrix (largest component)
    if hasattr(net.WC, 'data'):  # Sparse matrix
        wc_bytes = net.WC.data.nbytes + net.WC.indices.nbytes + net.WC.indptr.nbytes
        print(f"  WC (sparse): {wc_bytes/1e6:.1f} MB, nnz={net.WC.nnz:,}, dtype={net.WC.dtype}")
    else:
        wc_bytes = net.WC.nbytes
        print(f"  WC (dense): {wc_bytes/1e6:.1f} MB, dtype={net.WC.dtype}")
    total_bytes += wc_bytes
    
    # Change of basis matrices
    c_bytes = net.C.nbytes + net.C_T.nbytes
    print(f"  C + C_T: {c_bytes/1e6:.1f} MB, shape={net.C.shape}, dtype={net.C.dtype}")
    total_bytes += c_bytes
    
    # State vectors
    vec_bytes = net.actC.nbytes + net.bC.nbytes + net.extC.nbytes + net.estr.nbytes
    print(f"  Vectors (actC, bC, extC, estr): {vec_bytes/1e6:.1f} MB, dtype={net.actC.dtype}")
    total_bytes += vec_bytes
    
    # Optimizer states (if initialized)
    if hasattr(net, 'optim') and net.optim:
        opt_bytes = 0
        for key, val in net.optim.items():
            if hasattr(val, 'nbytes'):
                opt_bytes += val.nbytes
            elif hasattr(val, 'data'):  # Sparse
                opt_bytes += val.data.nbytes + val.indices.nbytes + val.indptr.nbytes
        print(f"  Optimizer states: {opt_bytes/1e6:.1f} MB")
        total_bytes += opt_bytes
    
    print(f"  {'─'*40}")
    print(f"  TOTAL ESTIMATED: {total_bytes/1e6:.1f} MB ({total_bytes/1e9:.2f} GB)")
    
    # Compare with float64 baseline
    if DTYPE_STORAGE == np.float32:
        float64_estimate = total_bytes * 2
        savings = float64_estimate - total_bytes
        print(f"  Float64 baseline would be: {float64_estimate/1e6:.1f} MB")
        print(f"  MEMORY SAVED: {savings/1e6:.1f} MB ({savings/float64_estimate*100:.0f}%)")
    print(f"{'='*60}\n")
...
def debug_wc_structure(net, treelet_rules=None, check_bindings=None):
    ...
    if is_sparse:
        ...
        print(
            f"  WC sparsity: {100 * (1 - net.WC.nnz / (net.WC.shape[0] * net.WC.shape[1])):.4f}%")
    else:
        ...
        for treelet in treelet_rules:
            if len(treelet) >= 2:
                ...
                try:
                    ...
                    p_idx = net.find_bindings_fast(
                        [b for b in net.binding_names if b.split('/')[0].startswith(parent_filler)])
                    l_idx = net.find_bindings_fast(
                        [b for b in net.binding_names if b.split('/')[0].startswith(left_filler)])
                    ...
                    if len(p_idx) > 0 and len(l_idx) > 0:
                        ...
                        for pi in p_idx[:5]:  # Check more bindings
                            for li in l_idx[:5]:
                                ...
                                if w != 0:
                                    weights_pl.append(
                                        (net.binding_names[pi], net.binding_names[li], w))

                        if weights_pl:
                            print(
                                f"    {parent} <-> {left}: Found {len(weights_pl)} non-zero weights")
                            for p, l, w in weights_pl[:5]:
                                print(f"      {p} <-> {l}: {w:.4f}")
                        else:
                            print(
                                f"    {parent} <-> {left}: NO WEIGHTS FOUND!")
                    else:
                        print(
                            f"    {parent} <-> {left}: Bindings not found (p_idx={len(p_idx)}, l_idx={len(l_idx)})")
        ...
        for bname in check_bindings:
            ...
            if len(idx_list) > 0:
                ...
                if is_sparse:
                    ...
                    row_sum = net.WC[idx, :].sum()
                print(
                    f"    {bname} (idx={idx}): row_nnz={row_nnz}, col_nnz={col_nnz}, row_sum={row_sum:.4f}")
            else:
...
def debug_parse_comparison(net, sent, target_bnames):
    ...
    print(f"\n  Final parse (grid point): {gp[:10]}..." if len(
        gp) > 10 else f"\n  Final parse: {gp}")
...
class GscNet():
    ...
    def __init__(self, hg=None, encodings=None, opts=None, qpolicy=None, seed=None,
        ...
        if seed is not None:
            ...
            print(
                f"GscNet.__init__: np.random.seed({seed}) called, position={state[2]}")
        ...
        if self.use_jax:
            ...
            if self.opts['use_sparse_wc']:
                ...
                print(f"  Using dok_matrix for memory-efficient construction (dtype={DTYPE_SPARSE.__name__})...")
                self.WC = sparse.dok_matrix(
                    (self.num_bindings, self.num_bindings), dtype=DTYPE_SPARSE)
            ...
            self.bC = np.zeros(self.num_bindings, dtype=DTYPE_STORAGE)
            self.estr = self.opts['init_estr'] * np.ones(self.num_bindings, dtype=DTYPE_STORAGE)
        ...
        if preload_wc is not None:
            ...
            self.estr = DTYPE_STORAGE(self.opts['init_estr']) * np.ones(self.num_bindings, dtype=DTYPE_STORAGE)
            ...
            if self.opts['use_sparse_wc']:
                ...
                self.WC = sp.csr_matrix((self.num_bindings, self.num_bindings), dtype=DTYPE_SPARSE)
                self.use_sparse = True
            else:
                self.WC = np.zeros((self.num_bindings, self.num_bindings), dtype=DTYPE_STORAGE)
                self.use_sparse = False
            self.bC = np.zeros(self.num_bindings, dtype=DTYPE_STORAGE)
            self.estr = DTYPE_STORAGE(self.opts['init_estr']) * np.ones(self.num_bindings, dtype=DTYPE_STORAGE)
        elif hg is not None:
        ...
        if self.use_jax:
            ...
            # NumPy version: CPU arrays (using DTYPE_STORAGE for memory efficiency)
            self.extC = np.zeros(self.num_bindings, dtype=DTYPE_STORAGE)
            self.ext = self.C2N(actC=self.extC)
            self._set_bowl_parameters()

            self.q = (self.opts['q_init'] * np.ones(self.num_roles)).astype(DTYPE_STORAGE)
            self.T = self.opts['T_init']
            self.dt = self.opts['dt_init']

            # Add state variables
            self.t = 0.
            self.actC = np.zeros(self.num_bindings, dtype=DTYPE_STORAGE)
            self.actCmat = self.vec2mat(self.actC)
            self.act = self.C2N()
            self.update_scale_constants(pos=0)
    ...
    def _add_change_of_basis_matrices(self):
        ...
        # Convert to appropriate dtype for memory efficiency
        if self.use_jax:
            ...
            # CPU path: convert to DTYPE_STORAGE for memory efficiency
            c_original_bytes = self.C.nbytes / 1e6
            self.C = self.C.astype(DTYPE_STORAGE)
            self.C_T = self.C_T.astype(DTYPE_STORAGE)
            self.N = self.N.astype(DTYPE_STORAGE)
            c_new_bytes = self.C.nbytes / 1e6
            print(f"  ✓ C matrix on CPU: {self.C.shape} ({self.C.dtype}, {c_original_bytes:.1f}MB → {c_new_bytes:.1f}MB)")
    ...
    def _build_model(self):
        ...
        if is_sparse:
            print(f"  Initializing WC as LIL matrix for efficient construction (dtype={DTYPE_SPARSE.__name__})...")
            self.WC = sparse.lil_matrix(
                (self.num_bindings, self.num_bindings), dtype=DTYPE_SPARSE)
        else:
            self.WC = np.zeros((self.num_bindings, self.num_bindings), dtype=DTYPE_STORAGE)
        ...
        if hasattr(self, 'use_sparse') and self.use_sparse:
            ...
            # FIX: Remove explicit zeros to match dense matrix structure
            # When setting weight=0 in lil_matrix, it still stores the entry
            # This causes structural differences vs dense (which has implicit zeros)
            nnz_before = self.WC.nnz
            self.WC.eliminate_zeros()
            nnz_after = self.WC.nnz
            if nnz_before != nnz_after:
                print(
                    f"      Removed {nnz_before - nnz_after} explicit zeros from WC")

            total = self.num_bindings ** 2
            sparsity = 100 * (1 - nnz_after / total)
            print(
    ...
    def _set_bowl_parameters(self):
        ...
        if isinstance(self.opts['bowl_center'], numbers.Number):
            if self.use_jax:
                ...
                self.bowl_center = (self.opts['bowl_center'] *
                                    np.ones(self.num_bindings, dtype=DTYPE_STORAGE))
        else:
            if self.use_jax:
    ...
    def update_scale_constants(
        ...
        if self.hg.opts['role_system'] == 'brick_role':
            ...
            weights = np.ones(self.num_bindings, dtype=DTYPE_STORAGE)
            ...
            if q_only:
                if self.use_jax:
                    ...
                    self.scale_constants = np.ones(self.num_bindings, dtype=DTYPE_STORAGE)
    ...
    def run_word(self, fname, wpos, symmetric=True, update_q_discrete=False, log_trace=False, debug_dynamics=False):
        ...
        if debug_dynamics:
            ...
            print(
                f"    extC sum={self.extC.sum():.4f}, max={self.extC.max():.4f}, nnz={np.count_nonzero(self.extC)}")
            ...
            print(
                f"    WC.dot(actC): sum={wc_contrib.sum():.4f}, max={wc_contrib.max():.4f}")
            # Check which bindings are getting input
            input_indices = np.where(self.extC > 0)[0]
            if len(input_indices) > 0:
                input_bnames = [self.binding_names[i]
                                for i in input_indices[:5]]
        ...
        if update_q_discrete:
            update_q = False
            self.q = (self.qpolicy[wpos] * np.ones(self.num_roles)).astype(DTYPE_STORAGE)
        else:
            update_q = True
        ...
        if debug_dynamics:
            ...
            top_bnames = [(self.binding_names[i], self.actC[i])
                          for i in top_idx]
    ...
    def run_wrapup(self, update_q_discrete=False, log_trace=False, clear_input=True):
        ...
        if update_q_discrete:
            update_q = False
            self.q = (self.opts['q_max'] * np.ones(self.num_roles)).astype(DTYPE_STORAGE)
        else:
            update_q = True
        ...
        if hasattr(self, '_debug_wrapup_first_call') and not self._debug_wrapup_first_call:
            ...
            print(
                f"        [run_wrapup: dur={integration_dur:.1f}, steps~{num_steps:.0f}, use_runC={self.opts['use_runC']}]")
    ...
    def initialize(self, train_opts=None):
        ...
        if self.train_opts['optimizer'] == 'adam':
            ...
            if hasattr(self, 'use_sparse') and self.use_sparse:
                ...
                # NOTE: Using DTYPE_SPARSE for memory efficiency
                self.optim['M_WC'] = sparse.csr_matrix(
                    self.WC.shape, dtype=DTYPE_SPARSE)
                self.optim['R_WC'] = sparse.csr_matrix(
                    self.WC.shape, dtype=DTYPE_SPARSE)
            else:
                # Dense matrices - use same dtype as WC
    ...
    def get_mask0(self):
        ...
        if self.train_opts['update_gram_only']:  # default False
            ...
            if hasattr(self, 'use_sparse') and self.use_sparse:
                ...
                # NOTE: Using DTYPE_SPARSE for memory efficiency
                mask0 = sparse.csr_matrix(self.WC.shape, dtype=DTYPE_SPARSE)
                ...
                for batch_start in range(0, total_roles, batch_size):
                    ...
                    for ri in range(batch_start, batch_end):
                        ...
                        if len(idx) > 0:
                            ...
                            rows_self, cols_self = np.meshgrid(
                                idx_array, idx_array, indexing='ij')
                            row_list.append(rows_self.ravel())
                            col_list.append(cols_self.ravel())
                        if not self.hg.roles.role_is_terminal[ri]:
                            indices = self.get_role_and_daughter_indices_fast(
                                ri)
                            if indices != None:
                                ...
                                row_list.extend(
                                    [rows_pl.ravel(), cols_pl.ravel()])
                                col_list.extend(
                                    [cols_pl.ravel(), rows_pl.ravel()])

                                # idx × idx_r (parent-right) + symmetric
                                rows_pr, cols_pr = np.meshgrid(
                                    idx, idx_r, indexing='ij')
                                row_list.extend(
                                    [rows_pr.ravel(), cols_pr.ravel()])
                                col_list.extend(
                                    [cols_pr.ravel(), rows_pr.ravel()])
                    ...
                    if row_list:
                        ...
                        batch_data = np.ones(len(batch_rows), dtype=DTYPE_SPARSE)
                ...
                print(
                    f"      mask0 construction: {time.time() - t_start:.2f}s")
                print(
                    f"      mask0 non-zero entries: {np.count_nonzero(mask0):,}")
    ...
    def train2(self, prefix_list=None, prefix_weights=None,
        ...
        if hasattr(self, 'use_sparse') and self.use_sparse:
            # FIX: Remove any explicit zeros that may have been added after initial eliminate_zeros()
            # (e.g., from setdiag() in bias2weight() which can store explicit zeros)
            nnz_before = self.WC.nnz
            self.WC.eliminate_zeros()
            nnz_after = self.WC.nnz
            if nnz_before != nnz_after:
                print(
                    f"PRE-TRAINING: Removed {nnz_before - nnz_after} explicit zeros from WC")

            # wc_dense = self.WC.toarray()
            wc_checksum = self.WC.sum()
            wc_abssum = np.abs(self.WC).sum()
            print(
                f"INIT WC (SPARSE): sum={wc_checksum:.10f}, abssum={wc_abssum:.10f}, nnz={self.WC.nnz}")
        else:
            wc_checksum = self.WC.sum()
            wc_abssum = np.abs(self.WC).sum()
            wc_nnz = np.count_nonzero(self.WC)
            print(
                f"INIT WC (DENSE): sum={wc_checksum:.10f}, abssum={wc_abssum:.10f}, nnz={wc_nnz}")
        print(f"INIT bC: sum={bc_checksum:.10f}, abssum={bc_abssum:.10f}")
        print(f"\n{'='*70}")
        print(
            f"STARTING TRAINING LOOP: {self.train_opts['num_epochs']} epochs")
        print(f"{'='*70}")
        for _ in range(self.train_opts['num_epochs']):
            ...
            print(
                f"  Initializing gradient arrays... (sparse={hasattr(self, 'use_sparse') and self.use_sparse})")

            if self.train_opts['parallel_parser_train']:
                print(
                    f"  Parallel parser training enabled - calling train_parallel_parsing()...")
            ...
            for pi, prefix in enumerate(prefix_list):
                print(
                    f"  Prefix {pi+1}/{len(prefix_list)}: {prefix}, weight={prefix_weights[pi]}")
                ...
                if prefix_weights[pi] > 0:
                    ...
                    _timing['estimate_prob'] = _timing.get(
                        'estimate_prob', 0) + (time.time() - _t_prob_start)
                    ...
                    _timing['cost_grad'] = _timing.get(
                        'cost_grad', 0) + (time.time() - _t_grad_start)
                    ...
                    if hasattr(self, 'use_sparse') and self.use_sparse and not self.use_jax:
                        ...
                        scaled_grad = dWC_curr * \
                            (scale_dWC * prefix_weights[pi])
                ...
                for rid in role_idx_list:
                    ...
                    if self.use_jax:
                        ...
                        if hasattr(self, 'use_sparse') and self.use_sparse:
                            ...
                            # NOTE: Using DTYPE_SPARSE for memory efficiency (mask is just 0/1)
                            rows, cols = np.meshgrid(idx, idx, indexing='ij')
                            mask_coo = sparse.coo_matrix(
                                (np.ones(len(idx)**2, dtype=DTYPE_SPARSE),
            ...
            if self.train_opts['update_w']:
                ...
                if not (('bias2_only' in self.train_opts) and self.train_opts['bias2_only']):
                    if self.train_opts['optimizer'] == 'adam':
                        if self.use_jax:
                            ...
                            if hasattr(self, 'use_sparse') and self.use_sparse:
                                ...
                                if m_coo.nnz > 0:
                                    ...
                                    r_sqrt_data = np.sqrt(
                                        r_coo.data) + self.optim['eps']
                                    update_data = self.train_opts['lrate'] * \
                                        m_coo.data / r_sqrt_data
                        ...
                        # FIX: Remove explicit zeros after Adam update to match dense behavior
                        if hasattr(self, 'use_sparse') and self.use_sparse:
                            self.WC.eliminate_zeros()
                        self._set_weights()
                    else:
                        ...
                        # FIX: Remove explicit zeros after SGD update to match dense behavior
                        if hasattr(self, 'use_sparse') and self.use_sparse:
                            self.WC.eliminate_zeros()
                        self._set_weights()
            ...
            if self.epoch_num % report_cycle == 0:
                ...
                print(
                    f"    TIMING: total={_timing['total']:.1f}s | estimate_prob={_t_prob:.1f}s ({100*_t_prob/_timing['total']:.0f}%) | cost_grad={_t_grad:.1f}s ({100*_t_grad/_timing['total']:.0f}%) | other={_t_other:.1f}s")
    ...
    def estimate_prob_inc(self, prefix, num_trials=40, progress=0, update_q_discrete=False):
        # NOW
        print(
            f"      >> estimate_prob_inc called: {num_trials} trials, progress={progress}")
        corpus = {}
        corpus['target'] = []
        corpus['count'] = []
        corpus['prob_sent'] = []
    ...
    def cost_grad(self, err, extC_token):
        ...
        if self.train_opts['bias1_only']:
            ...
            # FIX: For sparse, use dictionary to aggregate gradients by position
            # This ensures same behavior as dense += which accumulates incrementally
            if hasattr(self, 'use_sparse') and self.use_sparse and not self.use_jax:
                grad_dict = {}  # (row, col) -> accumulated value

            # DIAGNOSTIC: Track gradient statistics for debugging
            _diag_tree_count = 0
            _diag_pos_count = 0
            _diag_val_sum = 0.0
                    ...
                    if key in keys_tree:
                        ...
                        if self.use_jax:
                            ...
                            if hasattr(self, 'use_sparse') and self.use_sparse:
                                ...
                                # <-- Use mask values directly
                                _diag_val_sum += (coef_val * mask_values).sum()
                                ...
                                # DIAGNOSTIC: Track statistics for dense
                                _diag_tree_count += 1
                                n_masked = np.count_nonzero(masked)
                                _diag_pos_count += n_masked
                                _diag_val_sum += val * \
                                    self.train_opts['coef']['trees'] * n_masked
            ...
            # DIAGNOSTIC: Print tree gradient statistics every 100 epochs
            if self.epoch_num % 100 == 1 or self.epoch_num <= 5:
                is_sparse = hasattr(self, 'use_sparse') and self.use_sparse
                mode = "SPARSE" if is_sparse else "DENSE"
                print(f"    [Epoch {self.epoch_num}] GRADIENT DIAG ({mode}): "
                      f"trees={_diag_tree_count}, positions={_diag_pos_count}, "
                      f"val_sum={_diag_val_sum:.6f}")

                # For sparse, also show grad_dict statistics
                if is_sparse and len(grad_dict) > 0:
                    dict_sum = sum(grad_dict.values())
                    dict_nnz = len(
                        [v for v in grad_dict.values() if abs(v) > 1e-10])
                    dict_pos_count = len(grad_dict)
                    print(f"      grad_dict: {dict_pos_count} unique positions, "
                          f"{dict_nnz} non-zero values, sum={dict_sum:.6f}")
                    # Check for small values that might underflow
                    small_vals = [v for v in grad_dict.values()
                                  if 0 < abs(v) < 1e-10]
                    if small_vals:
                        print(
                            f"      WARNING: {len(small_vals)} values smaller than 1e-10!")
                    ...
                    if key in keys_treelet:
                        ...
                        if not self.train_opts['bias_only']:
                            if self.use_jax:
                                ...
                                if hasattr(self, 'use_sparse') and self.use_sparse:
                                    # FIX: Use dictionary to accumulate
                                    for pos in [(key[0], key[1]), (key[1], key[0]),
        ...
        # Build sparse WC gradient from accumulated dictionary (ONCE!)
        # FIX: Using dictionary ensures proper aggregation of duplicate entries
        # (same behavior as dense += which accumulates incrementally)
        if not self.use_jax and hasattr(self, 'use_sparse') and self.use_sparse:
            if len(grad_dict) > 0:
                # Convert dictionary to COO format (no duplicates now!)
                grad_rows = []
                grad_cols = []
                grad_vals = []
                for (r, c), v in grad_dict.items():
                    grad_rows.append(r)
                    grad_cols.append(c)
                    grad_vals.append(v)
                ...
                if self.epoch_num % 100 == 1 or self.epoch_num <= 5:
                    # Count positions that are currently zero vs non-zero in WC
                    wc_csr = self.WC.tocsr()
                    new_positions = 0
                    existing_positions = 0
                    new_grad_sum = 0.0
                    existing_grad_sum = 0.0
                    for (r, c), v in grad_dict.items():
                        if wc_csr[r, c] == 0:
                            new_positions += 1
                            new_grad_sum += abs(v)
                            ...
                            existing_grad_sum += abs(v)

                    print(
                        f"      SPARSE dWC: nnz={dWC.nnz}, sum={dWC.sum():.6f}")
                    print(
                        f"      Gradient targets: {existing_positions} existing WC positions (grad_sum={existing_grad_sum:.4f})")
                    print(
                        f"                        {new_positions} NEW positions (grad_sum={new_grad_sum:.4f})")
        ...
        # DIAGNOSTIC: For dense, print gradient sum and new vs existing positions
        if not self.use_jax and not (hasattr(self, 'use_sparse') and self.use_sparse):
            if self.epoch_num % 100 == 1 or self.epoch_num <= 5:
                dWC_nnz = np.count_nonzero(dWC)
                # Count positions that are currently zero vs non-zero in WC
                wc_nonzero_mask = self.WC != 0
                dwc_nonzero_mask = dWC != 0
                existing_positions = np.sum(wc_nonzero_mask & dwc_nonzero_mask)
                new_positions = np.sum((~wc_nonzero_mask) & dwc_nonzero_mask)
                existing_grad_sum = np.sum(
                    np.abs(dWC[wc_nonzero_mask & dwc_nonzero_mask]))
                new_grad_sum = np.sum(
                    np.abs(dWC[(~wc_nonzero_mask) & dwc_nonzero_mask]))

                print(f"      DENSE dWC: nnz={dWC_nnz}, sum={dWC.sum():.6f}")
                print(
                    f"      Gradient targets: {existing_positions} existing WC positions (grad_sum={existing_grad_sum:.4f})")
                print(
                    f"                        {new_positions} NEW positions (grad_sum={new_grad_sum:.4f})")
                print(f"      Current WC nnz: {np.count_nonzero(self.WC)}")
    ...
    def HGradC(self, actC=None, q=None, debug_dynamics=False):
        ...
        # DIAGNOSTIC: Check if sparse WC.dot produces same result as dense
        if debug_dynamics and hasattr(self, 'use_sparse') and self.use_sparse:
            wc_result = self.WC.dot(actC)
            print(
                f"    HGradC DEBUG: WC.dot(actC) shape={wc_result.shape}, dtype={wc_result.dtype}")
            print(
