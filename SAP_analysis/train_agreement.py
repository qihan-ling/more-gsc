"""
Train a GSC model on the Agreement grammar with probabilities
extracted from the Berkeley parser (sm5).

NOTE: SBAR -> SBAR , and S -> SBAR S had 0 matches in the Berkeley grammar
(binarization mismatch). Edit Agreement_probs.txt to assign them probabilities
manually before running this script.

Usage:
    python SAP_analysis/train_agreement.py
"""
...
PROBS_PATH = os.path.join('SAP_stimuli', 'Agreement_probs.txt')
SAVE_PREFIX = 'SAP_analysis/agreement'
...
# plt.title('Agreement Parsing Accuracy by Sentence Type', fontsize=14)
...
# TREELET_SENTENCES = [
#     'DT NN VBZ DT NN',   # grammatical: NP_sg + VP_sg (singular agreement)
#     'DT NNS VBZ DT NN',  # violation:   NP_pl + VP_sg (number disagreement)
# ]
# TREELET_ROLES = ['(1,2)', '(2,1)'] # (1,2) = subject head: NN vs NNS; (2,1) = subject NP: NP_sg vs NP_pl
# NUM_TREELETS = 10
