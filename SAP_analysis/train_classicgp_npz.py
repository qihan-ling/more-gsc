"""
Train a GSC model on the ClassicGP_NPZ grammar with probabilities
extracted from the Berkeley parser (sm5).

NOTE: SBAR -> SBAR , and S -> SBAR S had 0 matches in the Berkeley grammar
(binarization mismatch). Edit ClassicGP_NPZ_probs.txt to assign them
probabilities manually before running this script.

Usage:
    python SAP_analysis/train_classicgp_npz.py
"""
...
PROBS_PATH = os.path.join('SAP_stimuli', 'ClassicGP_NPZ_probs.txt')
SAVE_PREFIX = 'SAP_analysis/classicgp_npz'
...
net.generate_corpus_exhaustive()
...
print("\n" + "=" * 70)
print("Training ClassicGP_NPZ model...")
print("=" * 70)
...
plt.title('ClassicGP_NPZ Parsing Accuracy by Sentence Type', fontsize=14)
...
TREELET_SENTENCES = [
    'IN DT NN VBD , DT NN VBD JJ NN',
    'IN DT NN VBD DT NN VBD JJ NN',
]
TREELET_ROLES = ['(2,4)', '(7,2)'] # (2,4) checks clause boundary, (7,2) checks if SBAR is present
NUM_TREELETS = 10
