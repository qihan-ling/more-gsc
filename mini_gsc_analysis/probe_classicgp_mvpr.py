"""Probe the trained Classic-GP MVPR model.

Target pair (one):
    A: 'DT NN WP VBD VBN DT NN VBD JJ NN'   (d=8, len=10)
    B: 'DT NN VBN DT NN VBD JJ NN'           (d=6, len=8)
spillover = 2.
"""
...
def make_target_pairs() -> list[TargetPair]:
    return [
        TargetPair(
            name="mvpr_full_vs_reduced",
            sentence_a=pos_tokens("DT NN WP VBD VBN DT NN VBD JJ NN"),
            sentence_b=pos_tokens("DT NN VBN DT NN VBD JJ NN"),
            d_a=8, d_b=6, spillover=2,
        ),
    ]
